...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $currentTab == 'add-new-listing') {
    ...
    $formData = [
        ...
        //     array(
        //       "ImageOf" => "Attic",
        //       "MediaURL" => "https://nexus-mls.cloudnineweb.dev/wp-content/uploads/image1.jpg"
        //     ),
        //     array(              
        //       "ImageOf" => "Attic",
        //       "MediaURL" => "https://nexus-mls.cloudnineweb.dev/wp-content/uploads/image2.jpg"
        //     )
        // )
    ];
    ...
    if ($err) {
        ...
        if($jsonResponse['@odata.editLink']){
            echo '<p style="color: green;">Listing creaed successfully</p>';
        }
    }
}
...
<?php if($currentTab == 'add-new-listing'): ?>
    ...
    <?php wp_nonce_field('media-upload', 'media_upload_nonce'); ?>
        ...
        <div class="upload-field">
            ...
            <input type="file" name="Media" id="mediaFiles" multiple accept="image/*">
...
document.getElementById('mediaFiles').addEventListener('change', async function(e) {
    ...
    uploadPreview.innerHTML = '<div class="upload-loading">Uploading...</div>';
    
    for(let file of files) {
        // Validate file type
        if(!file.type.startsWith('image/')) {
            uploadPreview.innerHTML += `<div class="upload-error">Invalid file type: ${file.name}</div>`;
            continue;
        }
        
        // Validate file size (5MB max)
        if(file.size > 5 * 1024 * 1024) {
            uploadPreview.innerHTML += `<div class="upload-error">File too large: ${file.name}</div>`;
            continue;
        }
        ...
        try {
            ...
            if(data.success && data.data && data.data.url) {
                ...
                // Update preview
                uploadPreview.innerHTML = mediaUrls.map(item => `
                    <div class="preview-item">
                        <img src="${item.MediaURL}" width="100" alt="Preview">
                        <span>${fileName}</span>
                        <button type="button" class="remove-image" data-url="${item.MediaURL}">Remove</button>
                    </div>
                `).join('');
                
                // Update formData.Media
            ...
            uploadPreview.innerHTML += `<div class="upload-error">Failed to upload ${file.name}: ${error.message}</div>`;
        }
    }
});
...
document.getElementById('uploadPreview').addEventListener('click', function(e) {
    if(e.target.classList.contains('remove-image')) {
        ...
        window.mediaData = window.mediaData.filter(item => item.MediaURL !== url);
        e.target.closest('.preview-item').remove();
        console.log('Updated Media URLs:', window.mediaData);
    }
});
</script>
